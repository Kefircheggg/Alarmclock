...
void loop() {
  ...
  calibration = EEPROM.read(0x60); //Выставление значений переменным в зависимости от EEPROM
  alarmtimehour = EEPROM.read(0x50); //Выставление значений переменным в зависимости от EEPROM
  alarmtimeminute = EEPROM.read(0x40); //Выставление значений переменным в зависимости от EEPROM
  alarm = EEPROM.read(0x30); //Выставление значений переменным в зависимости от EEPROM
  stb_time = EEPROM.read(0x20); //Выставление значений переменным в зависимости от EEPROM
  ShowDate = EEPROM.read(0x10); //Выставление значений переменным в зависимости от EEPROM
...
if(clockmode == 2) { //Меню 2 - меню с установками будильника
  ...
  */
  EEPROM.write(0x40,alarmtimeminute0); 
  EEPROM.write(0x50,alarmtimehour0);
  ...
  if(KB.isPressed()) {
    ...
    if(KB.getNum == 15 && alarm == false) {
      while(true) {  //Цикл подтверждения установки будильника
        ...
        if(KB.onPress()) { //Если нажата кнопка
          ...
          EEPROM.write(0x30,true); //Включаем будильник
          ...
          EEPROM.write(0x30,false); //Не включаем будильник
          lcd.clear(); //Очищаем дисплей
    ...
    if(KB.getNum == 15 && alarm == true && clockmode == 2) { //Если будильник уже включен
      while(true) { //Запускаем цикл подтверждения выключения будильника
        ...
        if(KB.onPress()) { //Если нажата кнопка
          if(KB.getNum == 15) { //Если кнопка - #
          EEPROM.write(0x30,false); //Выключаем будильник
          lcd.clear(); //Очищаем дисплей
  ...
  */
  ...
  if(possettings == 0) { //Это система выставления > в зависимости от позиции  
    lcd.print(">Time light"); //Отображаем информацию
      if(KB.isPressed()) { //Если кнопка нажата
      if(KB.getNum == 7) { EEPROM.write(0x20, 5); } //Если кнопка - 7, то выставить 5 секунд
      if(KB.getNum == 8) { EEPROM.write(0x20, 10); } //Если кнопка - 8, то выставить 10 секунд
      if(KB.getNum == 9) { EEPROM.write(0x20, 20); } //Если кнопка - 9, то выставить 20 секунд
    }
  ...
  if(possettings == 1) { //Если пользователь нажал 0
    if(KB.isPressed()) { //Если кнопка нажата
      if(KB.getNum == 7) { EEPROM.write(0x10, false); } //Если кнопка - 7, то опустить флажок и запиисать в EEPROM 0
      if(KB.getNum == 9) { EEPROM.write(0x10, true); } //Если кнопка - 9, то поднять флажок и запиисать в EEPROM 1
    }
    lcd.print(" Time light");  //Отображаем информацию
    lcd.setCursor(0,1); //Ставим курсор
    lcd.print(">Show date  "); //Отображаем информацию
        ...
        while(true) { //Входим в цикл выбора: сбросить, или нет
          ...
          if(KB.onPress()) { //Если кнопка нажата 
            ...
            if(KB.getNum == 5 && resetpos == 1) { //Если кнопка - 5, и положение YES 
                ...
                EEPROM.write(0x60,0); 
                delay(200); 
              EEPROM.write(0x60,true); //Поднимаем флажок настройки
...
if(KB.isPressed()) { //Если нажата кнопка
    ...
    if(alarm == true) { //Если флажок будильника поднят
      if(alarmtimehour == now.hour()) { //Если час срабатывания настал
        if(alarmtimeminute == now.minute()) { //Если минута срабатывания настала 
          if(clockmode == 1 || clockmode == 3 || clockmode == 4) { //Если меню 1 или 3 или 4
  ...
  void alarmFunction() { //Это функция будильника, тут можно творить что угодно
            ...
            tone(buzzer_pin); //Пищим пищалкой
            tone(buzzer_pin); //Пищим пищалкой
            if(KB.onPress()) { //Если кнопка нажата
              if(KB.getNum == 14) {
                EEPROM.write(0x40, now.minute() + 5);
                lcd.clear();
              ...
              if(KB.getNum == passalarm0) { //Если нажатая кнопка равна рандомной кнопке
                ...
                EEPROM.write(0x30, false); //Опускаем флажок будильника
                for(int i = 255; i>255; i--) {
                  analogWrite(led_pin, i);
                  analogWrite(Backlight_pin, i);
                  delay(1);
            ...
            digitalWrite(led_pin, LOW);
            delay(50); //задержка
            if(EEPROM.read(0x30) == false) { break; }
    }  
  }
  ...
  void Calibrationfunction() { //Это функция первоначальной настройки
        ...
        if(KB.onPress()) { //Если нажата кнопка 
          ...
          if(KB.getNum == 5) { //Если кнопка - 5, то записываем в EEPROM 1 или 0
            if(Showdatepos == 0) { //Если была нажата кнопка 4, то
              EEPROM.write(0x10,true); //Записываем в EEPROM 1
              lcd.clear(); //Очищаем дисплей
            ...
            if(Showdatepos == 1) { //Если была нажата кнопка 6, то
              EEPROM.write(0x10,false); //Записываем в EEPROM 0
              lcd.clear(); //Очищаем дисплей
        ...
        if(KB.onPress()) { //При нажатии кнопки 
          ...
          if(KB.getNum == 8) { //Если кнопка - 8 
            if(lightpos == 0) { //То в зависимости от ранее нажатой кнопки 
              EEPROM.write(0x20, 5); //Ставим либо 5, 10 или 20 секунд
              lcd.clear(); //Очищаем дисплей
    ...
    pressfornext(5); //см. строку 373
    EEPROM.write(0x60, false); //Опускаем флажок настройки
    digitalWrite(Backlight_pin,LOW); //Выключаем подсветку
